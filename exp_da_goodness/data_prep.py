...
def daPictureConcatenate(kanri_no):
...
if __name__ == '__main__':
    ...
    parser.add_argument("--make", action="store_true", help="augmented data creation & concat whole.")
    parser.add_argument("--create_da_pics", action="store_true", help="augmented data creation.")
    parser.add_argument("--create", default=32, type=int, help="concat augmented data.")
    parser.add_argument("--clean", action="store_true", help="remove auged datas.")

    args = parser.parse_args()
    
    if args.create_da_pics:
        CreateDaPictures()
    if args.create < 32:
        daPictureConcatenate(args.create)
    if args.make:
        concatWhole()
    if args.clean:
        clean()
